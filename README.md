...
Ei lesing av eit heilt uttrekk går ofte over natta eller helga, og medan ein jobb køyrer ber appen Windows om å ikkje gå i dvale. Skjermen får framleis sløkkje seg. Sperra blir fjerna med ein gong jobben er ferdig, avbroten eller feila, og ho forsvinn uansett når appen blir lukka.
...
Fyrste gongen ein ny versjon blir opna, viser appen **Kva er nytt** av seg sjølv. Lista kan hentast fram att når som helst, frå **Hjelp → Kva er nytt ...** eller frå lenka nedst til høgre i statuslinja, som òg finst i browser-appen. Notata ligg i [`nbrenamer/web/endringar.json`](nbrenamer/web/endringar.json) og blir skrivne for arkivaren: kva som er annleis å bruke, ikkje kva som er endra i koden. Appen hugsar det øvste versjonsnummeret i lista i `localStorage`, så autovisinga følgjer notata og ikkje versjonsnummeret i `package.json`.
...
**1. Sett nytt versjonsnummer, og skriv endringsnotatet.** Legg ei ny blokk øvst i `versjonar` i [`nbrenamer/web/endringar.json`](nbrenamer/web/endringar.json), med same nummer som releasen. Det er denne lista brukaren får opp fyrste gongen han opnar den nye versjonen, så ho er ein del av releasen, ikkje noko som kan kome etterpå. Køyr så `npm version` inne i `desktop`, så blir både `package.json` og `package-lock.json` oppdaterte. Bygget les versjonen frå `package.json`. `package-lock.json` har han òg, to stader, og står dei ulikt, blir lockfila skriven om av neste `npm install`, midt i ei heilt anna endring.
...
| `rotation` | rotasjonen som gjer ID-en leseleg (0/90/180/270), altså den gjennomgangsmodusen snur biletet med |
| `id_boks` | kvar i biletet ID-en stod, som `x0,y0,x1,y1` i brøkdelar av det oppretta biletet. Gjennomgangen bruker han til å vise lappen stor |
| `raw_id` | ID-en slik han stod i motivet. Står det `+` mellom to delar, låg taldelen og løpenummeret i kvar sitt tekstfelt, og appen batt dei saman fordi dei står inntil kvarandre |
...
- `--overwrite` skriv over eksisterande målfiler. Utan dette blir kollisjonar rapporterte og hoppa over. I appen blir slike kollisjonar talde opp før køyringa: stadfestinga seier kor mange av namna som alt er tekne i ut-mappa, og om dei blir skrivne over eller talde som konfliktar. Det er den einaste sjekken som ser filene frå ei tidlegare køyring; duplikatsjekken i steg 2 kjenner berre rapporten. Talet gjeld JPEG-ane, og krev at årsmappe-valet står likt i sjekken og i køyringa, for årsmappene avgjer kvar filene hamnar.
- `ok`-filer får nytt namn; `manuell_*`-filer blir kopierte til `_manuell\<status>\` med originalnamn.
- Køyringa legg att to lister, og til saman er dei kvitteringa på kva som skjedde. `omdøypte.csv` i ut-mappa har ei rad per fil som kom på plass, med kjeldesti, målsti, TIFF-en sine to stiar, Foto-ID og om fila blei kopiert eller flytta. Rada blir skriven med ein gong fila er på plass, ikkje til slutt, så lista er sann heilt fram til det sekundet ei køyring over natta måtte bli avbroten. Køyrer du på nytt mot same ut-mappa, blir radene lagde til. `_manuell\uidentifiserte.csv` er den andre halvparten: alle bilete som ikkje kunne namngivast, med grunngjeving og kva mappe dei blei kopierte til. Filer som kolliderte eller ikkje lét seg kopiere står der òg, med grunnen, slik at ingenting forsvinn utan spor.
...
- **Lesing i to steg**: fyrst finn detektoren alle tekstfelta i biletet, uansett kva veg dei står. Så blir kvart felt retta opp til eit rektangel og lese for seg, både slik det ligg og opp ned, og den lesinga OCR-en er tryggast på vinn. Det er dette som gjer at eit løpenummer som står opp ned blir lese som `0004` og ikkje `7000`, og at me ikkje treng å gjette på kva veg motivet ligg. Fyrste forsøket går på halv oppløysing fordi deteksjonen er det dyraste steget, mens utsnitta blir klipte frå det store biletet slik at attkjenninga får dei skarpe pikslane.
- **Ekstra forsøk**: berre når fyrste lesinga ikkje gir ein heil ID med god skår, prøver appen på nytt, billegaste forsøk fyrst. Fyrst det same biletet i **full oppløysing**, for ein liten lapp under motivet kan forsvinne heilt når biletet blir halvert: på ein slik lapp gav halv oppløysing eitt tekstfelt med skåren 0,215, mens full oppløysing las ID-en med 0,99. Så blir heile biletet **snudd** og prøvd på nytt (`--rotations`, standard `0,90,270,180`), som sikkerheitsnett for at deteksjonen kan gå glipp av ein lapp i éi stilling og finne han i ei anna. Alt dette kostar berre tid på bilete som ikkje alt har gitt ein sikker ID. Målt på 48 ekte skann: 0,42 s per bilete mot 0,33 utan dei ekstra forsøka, og to bilete som elles hadde hamna i `_manuell` fekk rett namn.
- **Val mellom fleire kandidatar**: same ID-en står ofte både handskriven i motivet og maskinskriven på ein lapp på tvers. Appen samlar alle kandidatane og vel mellom dei etter to reglar. Ein ID som står **heil i eitt tekstfelt** vinn alltid over ein som må settast saman av to felt, for då har OCR-en sjølv sett samanhengen. Skal to felt settast saman, må dei liggje **inntil kvarandre**, på same linje eller i same kolonne, målt mot tjukkleiken på teksten. Det er dette som skil løpenummeret på ein eigen klistrelapp frå datoen eller årstalet som står like ved. Løpenummeret må stå fyrst i sitt felt og vere fire siffer, men det kan ha meir tekst etter seg: på mange lappar les OCR-en `0042 Eigar: Johan B.Smørdal` som eitt felt, fordi han deler linja i mellomrommet etter punktumet. Blant like gode kandidatar vinn den med høgast OCR-skår. Skiljeteiknet kan vere punktum, komma eller bindestrek, og korkje `f`-en eller punktumet treng stå på lappen: dei lagar appen sjølv.
- **Dekodekø**: å dekode eit 92-megapikselskann tek 0,31 s og lesinga rundt 0,25, og gjekk dei etter kvarandre ville skjermkortet stått stille over halve tida. Dei neste bileta blir difor dekoda i eigne trådar mens OCR-en jobbar. Det same skjuler ventetida når filene blir henta over nettverk. Målt: 0,58 s per bilete mot 0,83 utan køen.
